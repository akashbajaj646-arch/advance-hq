...
let mcpSetToolName: string | null = null;
let mcpReqId = 0;
...
async function mcpEnsureReady(): Promise<{ ok: boolean; error?: string }> {
  if (!AM_MCP_KEY) {
    return { ok: false, error: 'AM_MCP_KEY env var not set — add the am_… API key (from your ApparelMagic MCP config) in Vercel.' };
  }
  if (!mcpInitialized) {
  ...
  if (!mcpSetToolName) {
    ...
    const setTool =
      ...
      tools.find(t => /target_qty/i.test(JSON.stringify(t.inputSchema || {})));
    if (!setTool) {
      const names = tools.filter(t => /invent|adjust/i.test(t.name)).map(t => t.name).join(', ');
      return { ok: false, error: `No inventory-set tool found via MCP tools/list. Inventory-ish tools: ${names || '(none)'}` };
    }
    mcpSetToolName = setTool.name;
  }
  return { ok: true };
}

function extractToolResult(msg: any): { raw: string; isError: boolean } {
  const isError = !!(msg?.error || msg?.result?.isError);
  const content = msg?.result?.content;
  if (Array.isArray(content)) {
    const raw = content.filter((c: any) => c.type === 'text').map((c: any) => c.text).join('\n');
    return { raw, isError };
  }
  return { raw: JSON.stringify(msg?.result ?? msg?.error ?? null), isError };
}
...
async function mcpSetInventory(skuId: number, targetQty: number, warehouseId: number, reason: string) {
  ...
  // Session may have expired between warm invocations — re-handshake once
  if (r.status === 404 || r.status === 400 || r.msg?.error?.code === -32000) {
  ...
  const result = extractToolResult(r.msg);
  if (r.status >= 400 || result.isError) {
    return { ok: false, error: `MCP tools/call failed: HTTP ${r.status} ${result.raw.slice(0, 300)}`, raw: result.raw };
  }
  return { ok: true, error: null as string | null, raw: result.raw };
}
...
export async function POST(request: Request) {
  ...
  if (action === 'submit') {
    ...
    await supabase.from('inventory_adjustments').insert({
      ...
      am_endpoint: `mcp:${mcpSetToolName || 'inventory-set'}`,
