...
// Two HMAC-signed credentials, no dependencies:
//   1. Link token     - single use, short lived, travels in a URL
//   2. Session cookie - issued once the link token is consumed
//
// A link may carry a payment request. The amount is signed into the token;
// the itemized invoice snapshot is stored server-side and fetched by jti.
//
...
const sign = (payload, kind) =>
  b64url(crypto.createHmac("sha256", secret()).update(`${kind}:${payload}`).digest());
...
/** pay: optional { amount (cents), currency, reference, invoiceNumber } */
export function createLinkToken(email, ttlSeconds = DEFAULT_LINK_TTL, pay = null) {
  ...
  if (pay && pay.amount > 0) {
    body.pay = {
      ...
      reference: String(pay.reference || "Wholesale order").slice(0, 160),
...
export function createSessionValue(email, pay = null, jti = null) {
...
export function verifySessionValue(value) {
  ...
  return d && d.email ? { email: d.email, pay: d.pay || null, jti: d.jti || null } : null;
}
