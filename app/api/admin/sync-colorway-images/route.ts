...
async function syncOneProduct(productId: string): Promise<{ images: number }> {
  ...
  for (const cw of colorways) {
    // AM's product_attributes records carry the colorway label in
    // `attribute_2` / `attr_2_name` (NOT `attr_2` like the rest of their API)
    const attr2 = cw.attribute_2 || cw.attr_2_name || cw.attr_2 || cw.color || cw.name || null;
    const paId = cw.product_attribute_id ? String(cw.product_attribute_id) : (cw.id ? String(cw.id) : null);

    let sort = 0;
    const pushImg = (url: any) => {
      if (url && !seen.has(url)) {
        seen.add(url);
        rows.push({
    ...
    // Singular primary image plus the images array (either can exist alone)
    if (cw.image && cw.image.img) pushImg(cw.image.img);
    if (cw.images && Array.isArray(cw.images)) {
      for (const img of cw.images) pushImg(img.img);
    }
  }
...
export async function GET(request: Request) {
  // Vercel cron invokes via GET: run the real cursor sync for the cron agent,
  // keep GET as a read-only probe for humans.
  const isCron = (request.headers.get('user-agent') || '').includes('vercel-cron');
  if (isCron) return runCursorSync();
  try {
