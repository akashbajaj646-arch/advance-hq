...
export default function WarehousePage() {
  ...
  const colorImages: string[] = (() => {
    if (!detail) return [];
    const cw = (detail.colorway_images || []).filter((i: any) => i.attr_2 === selectedColor);
    if (cw.length > 0) return cw.map((i: any) => i.image_url);
    // Fallback until colorway image sync has covered this product
    return (detail.images || []).map((i: any) => i.image_url);
  })();
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
                    {colors.map(c => (
                      <button
                        ...
                        className={`flex-shrink-0 px-5 py-3 rounded-xl text-base font-semibold border-2 transition-colors ${
                          selectedColor === c
                  ...
                  <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1">
                    {colorImages.map((url, i) => (
                      <button
                        ...
                        className="flex-shrink-0 w-36 h-36 sm:w-44 sm:h-44 rounded-xl overflow-hidden border border-gray-200 bg-white"
                      >
                        <img src={url} alt={`${detail.product.style_number} ${selectedColor} ${i + 1}`} className="w-full h-full object-cover" loading="lazy" />
                      </button>
                    ))}
                  </div>
                ) : (
