        ...
        {loading ? <div className="text-center py-8 text-gray-500">Loading...</div> : orders.length === 0 ? <div className="text-center py-8 text-gray-500">No orders found</div> : (
          <>
            <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-gray-200">{visibleColumns.map(col => <th key={col} className="table-header pb-3 whitespace-nowrap">{getColumnLabel(col)}</th>)}</tr></thead><tbody>
              {orders.map(o => (<tr key={o.id} className="border-b border-gray-100 hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => router.push(`/orders/${o.order_number}`)}>{visibleColumns.map(col => (<td key={col} className="table-cell text-sm max-w-[200px] truncate">{col === 'order_number' ? <span className="font-medium text-brand-600">{o[col]}</span> : col === 'order_status' ? <span className={`px-2 py-0.5 rounded text-xs font-medium ${o[col] === 'shipped' ? 'bg-green-100 text-green-700' : o[col] === 'open' ? 'bg-blue-100 text-blue-700' : o[col] === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}`}>{o[col] || '-'}</span> : fmt(col, o[col])}</td>))}</tr>))}
            </tbody></table></div>
            ...
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-gray-500"><button onClick={() => router.push(`/customers/${selected.apparel_magic_customer_id}`)} className="text-brand-600 hover:underline">{selected.customer_name || 'Unknown'}</button>{selected.po_number ? ` · PO: ${selected.po_number}` : ''}</p>
                ...
                <div className="flex items-center gap-2"><PrintButton entityType="order" onDownload={() => generateOrderPDF(selected, orderItems, 'download', [])} onPrint={() => generateOrderPDF(selected, orderItems, 'print', [])} /><button onClick={() => setSelected(null)} className="text-gray-400 hover:text-gray-600 text-2xl leading-none">×</button></div>
              </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {DETAIL_SECTIONS[detailTab].fields.map(field => {
                    ...
                    return (<div key={field} className={`rounded-lg p-3 ${hasValue ? 'bg-gray-50' : 'bg-gray-50/50'}`}><p className="text-xs text-gray-400 mb-1">{getColumnLabel(field)}</p>
                      {CLICKABLE[field] && hasValue ? <button onClick={() => { const t = CLICKABLE[field]; const v = field === 'customer_name' ? selected.apparel_magic_customer_id : String(value); const routes: any = {customer: '/customers/', order: '/orders/', invoice: '/invoices/', shipment: '/shipments/', pick_ticket: '/pick-tickets/'}; router.push((routes[t] || '/') + v); }} className="text-sm font-medium text-brand-600 hover:underline">{String(value)}</button>
