-- FALLBACK version — use ONLY if backorders_view.sql errors on the product_skus join.
-- Identical, minus the product_skus bin-location fallback.
...
select
  ...
  min(nullif(inv.bin_location, ''))                 as bin_location
from order_items oi
