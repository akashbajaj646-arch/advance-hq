        ...
        {loading ? <div className="text-center py-8 text-gray-500">Loading...</div> : invoices.length === 0 ? <div className="text-center py-8 text-gray-500">No invoices found</div> : (
          <>
            <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-gray-200">{visibleColumns.map(col => <th key={col} className="table-header pb-3 whitespace-nowrap">{getColumnLabel(col)}</th>)}</tr></thead><tbody>
              {invoices.map(inv => (<tr key={inv.id} className="border-b border-gray-100 hover:bg-gray-50 cursor-pointer transition-colors" onClick={() => router.push(`/invoices/${inv.invoice_number}`)}>{visibleColumns.map(col => (<td key={col} className="table-cell text-sm max-w-[200px] truncate">
                {col === 'invoice_number' ? <span className="font-medium text-brand-600">{inv[col]}</span>
                  : col === 'customer_name' ? <button onClick={(e) => { e.stopPropagation(); router.push(`/customers/${inv.apparel_magic_customer_id}`); }} className="text-brand-600 hover:underline">{inv[col]}</button>
                  : col === 'apparel_magic_order_id' && inv[col] ? <button onClick={(e) => { e.stopPropagation(); router.push(`/orders/${inv[col]}`); }} className="text-brand-600 hover:underline">{inv[col]}</button>
      ...
      {selected && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto"><div className="p-6">
            ...
            {detailTab === 'pts' && (<div className="overflow-x-auto">{relPTs.length === 0 ? <p className="text-gray-400 text-center py-8">No pick tickets</p> : (<table className="w-full text-sm"><thead><tr className="bg-gray-50 border-b border-gray-200"><th className="px-3 py-2 text-left font-medium text-gray-500">PT #</th><th className="px-3 py-2 text-left font-medium text-gray-500">Date</th><th className="px-3 py-2 text-right font-medium text-gray-500">Qty</th><th className="px-3 py-2 text-right font-medium text-gray-500">Total</th><th className="px-3 py-2 text-left font-medium text-gray-500">WMS</th><th className="px-3 py-2 text-left font-medium text-gray-500">Carton</th></tr></thead><tbody>{relPTs.map((pt, i) => (<tr key={i} className="border-b border-gray-100 hover:bg-gray-50 cursor-pointer" onClick={() => router.push(`/pick-tickets/${pt.pick_ticket_id}`)}><td className="px-3 py-2 font-medium text-brand-600 hover:underline">PT-{pt.pick_ticket_id}</td><td className="px-3 py-2">{pt.pick_ticket_date || '-'}</td><td className="px-3 py-2 text-right">{pt.qty || 0}</td><td className="px-3 py-2 text-right">${parseFloat(pt.total_amount || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td><td className="px-3 py-2"><span className={`px-2 py-0.5 rounded text-xs font-medium ${pt.wms_status === 'shipped' || pt.wms_status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{pt.wms_status || 'pending'}</span></td><td className="px-3 py-2 text-gray-500">{pt.carton_status || '-'}</td></tr>))}</tbody></table>)}</div>)}
